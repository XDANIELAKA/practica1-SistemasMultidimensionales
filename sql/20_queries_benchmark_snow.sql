...
FROM olap_snow.fact_reservas f
JOIN olap_snow.dim_tiempo t
    ON f.tiempo_sk = t.tiempo_sk
JOIN olap_snow.dim_hotel h
...
FROM olap_snow.fact_estancias f
JOIN olap_snow.dim_hotel h
    ON f.hotel_sk = h.hotel_sk
JOIN olap_snow.dim_habitacion hb
    ON f.habitacion_sk = hb.habitacion_sk
JOIN olap_snow.dim_tarifa ta
...
FROM olap_snow.fact_servicios f
JOIN olap_snow.dim_hotel h
    ON f.hotel_sk = h.hotel_sk
JOIN olap_snow.dim_cliente c
    ON f.cliente_sk = c.cliente_sk
JOIN olap_snow.dim_servicio s
...
JOIN olap_snow.dim_geografia g
    ON h.geografia_sk = g.geografia_sk
GROUP BY t.anio, t.trimestre, g.comunidad_autonoma
ORDER BY t.anio, t.trimestre, g.comunidad_autonoma;
